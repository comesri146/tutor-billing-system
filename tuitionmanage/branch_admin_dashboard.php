...
function isContactExists($conn, $branchID, $contact) {
    ...
    $stmt->bind_param("ss", $branchID, $contact);
    $stmt->execute();
...
if (isset($_POST['addStudent'])) {
    ...
    if (isContactExists($conn, $branchID, $contact)) {
        echo '<script>alert("Student with this contact number already exists!");</script>';
    } elseif (isStudentExists($conn, $branchID, $studentName, $contact, $parentName)) {
        echo '<script>alert("Student with these details already exists!");</script>';
    } else {
